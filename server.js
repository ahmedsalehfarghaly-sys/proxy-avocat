/**
 * proxy-avocat v4.3.5 – production
 *
 ...
 *   USE_SANDBOX        – "true" pour repasser en sandbox (défaut: production)
...
// Production par défaut. Passer USE_SANDBOX=true pour tester en sandbox.
const USE_SANDBOX = String(process.env.USE_SANDBOX || '').toLowerCase() === 'true';
...
function validateEnv() {
  ...
  if (!JD_CLIENT_ID)     missing.push('JD_CLIENT_ID (ou fallback LF_CLIENT_ID)');
  if (!JD_CLIENT_SECRET) missing.push('JD_CLIENT_SECRET (ou fallback LF_CLIENT_SECRET)');

  if (missing.length) {
    log.error('Variables manquantes: ' + missing.join(', ') + '. Les requêtes upstream échoueront.');
  } else {
...
async function getToken(clientId, clientSecret) {
  if (!clientId || !clientSecret) {
    throw new Error('Identifiants API manquants. Vérifiez LF_CLIENT_ID / LF_CLIENT_SECRET / JD_CLIENT_ID / JD_CLIENT_SECRET.');
  }
...
async function fetchWithRetry(url, options, clientId, clientSecret) {
  ...
  while (true) {
    ...
    if (res.status === 401 && attempt === 0) {
      log.debug('401 reçu, purge du cache token et retry');
...
function rerankContentieux(results) {
  const arr = Array.isArray(results) ? [...results] : [];
  return arr.sort((a, b) => LAYER_ORDER.indexOf(detectContentieuxLayer(a)) - LAYER_ORDER.indexOf(detectContentieuxLayer(b)));
}
...
async function resolveCodeId(codeTerms) {
  const list = await callLegifrance('/list/code', {
    codeName: codeTerms,
    pageSize: 3,
    pageNumber: 1,
    states: ['VIGUEUR'],
...
app.get('/lf/commit', async (_req, res) => {
  try {
    res.json({ ok: true, path_used: '/misc/commitId', ...(await callLegifrance('/misc/commitId', {}, 'GET')) });
  } catch (err) {
    handleError(res, err);
  }
});
...
app.get('/jd/healthcheck', async (_req, res) => {
  try {
    res.json({ ok: true, path_used: '/healthcheck', ...(await callJudilibre('/healthcheck')) });
  } catch (err) {
    handleError(res, err);
  }
});
  ...
  try {
    ...
    res.json({
      ...
      routing: {
        ...
        contentieux_layer: results[0] ? detectContentieuxLayer(results[0]) : null,
        endpoint_called: '/lf/search-simple',
      },
    });
  } catch (err) {
    handleError(res, err);
...
app.post('/lf/suggest/acco', async (req, res) => {
  ...
  try {
    res.json({ ok: true, path_used: '/suggest/acco', ...(await callLegifrance('/suggest/acco', { searchText })) });
  } catch (err) {
    handleError(res, err);
  }
});
...
app.post('/lf/suggest/pdc', async (req, res) => {
  ...
  try {
    const payload = {
      ...
      ...(req.body?.fond ? { fond: req.body.fond } : {}),
    };
  ...
  try {
    ...
    res.json({
      ...
      routing: {
        ...
        normalized_query: normalized,
        intent: 'ARTICLE',
        endpoint_called: '/lf/article-resolve',
      },
    });
  } catch (err) {
    handleError(res, err);
...
app.post('/lf/consult/article-by-eli', async (req, res) => {
  ...
  try {
    res.json({ ok: true, path_used: '/consult/getArticleWithIdEliOrAlias', idEliOrAlias, ...(await callLegifrance('/consult/getArticleWithIdEliOrAlias', { idEliOrAlias })) });
  } catch (err) {
    handleError(res, err);
  }
});
...
app.post('/lf/consult/same-num-article', async (req, res) => {
  ...
  try {
    res.json({ ok: true, path_used: '/consult/sameNumArticle', ...(await callLegifrance('/consult/sameNumArticle', { articleCid, articleNum, textCid, date: normalizeWhitespace(date || today()) })) });
  } catch (err) {
    handleError(res, err);
  }
});
...
app.post('/lf/search/canonical-version', async (req, res) => {
  ...
  try {
    res.json({ ok: true, path_used: '/search/canonicalVersion', ...(await callLegifrance('/search/canonicalVersion', { textId, date: normalizeWhitespace(date || today()) })) });
  } catch (err) {
    handleError(res, err);
  }
});
...
app.post('/lf/search/canonical-article', async (req, res) => {
  ...
  try {
    res.json({ ok: true, path_used: '/search/canonicalArticleVersion', ...(await callLegifrance('/search/canonicalArticleVersion', { articleId, date: normalizeWhitespace(date || today()) })) });
  } catch (err) {
    handleError(res, err);
  }
});
...
app.post('/lf/consult/dossier-legislatif', async (req, res) => {
  ...
  try {
    res.json({ ok: true, path_used: '/consult/dossierLegislatif', id, ...(await callLegifrance('/consult/dossierLegislatif', { id })) });
  } catch (err) {
    handleError(res, err);
  }
});
...
app.post('/lf/list/dossiers-legislatifs', async (req, res) => {
  ...
  try {
    res.json({ ok: true, path_used: '/list/dossiersLegislatifs', ...(await callLegifrance('/list/dossiersLegislatifs', { type, legislatureId: Number(legislatureId) })) });
  } catch (err) {
    handleError(res, err);
  }
});
...
app.post('/lf/consult/cnil', async (req, res) => {
  ...
  if (textId) {
    try {
      return res.json({ ok: true, mode: 'consult', path_used: '/consult/cnil', textId, result: await callLegifrance('/consult/cnil', { textId, ...(req.body?.searchedString ? { searchedString: req.body.searchedString } : {}) }) });
    } catch (err) {
  ...
  try {
    ...
    res.json({ ok: true, mode: 'search', path_used: '/search', totalResultNumber: upstream.totalResultNumber, results: upstream.results || [], routing: { query, fond_used: 'CNIL', endpoint_called: '/lf/consult/cnil' } });
  } catch (err) {
    handleError(res, err);
  }
});
...
app.post('/lf/consult/acco', async (req, res) => {
  ...
  if (id) {
    try {
      return res.json({ ok: true, path_used: '/consult/acco', id, result: await callLegifrance('/consult/acco', { id }) });
    } catch (err) {
      return handleError(res, err);
    }
  }
  ...
  try {
    ...
    res.json({ ok: true, mode: 'search', path_used: '/search', totalResultNumber: upstream.totalResultNumber, results: upstream.results || [], routing: { query, fond_used: 'ACCO', endpoint_called: '/lf/consult/acco' } });
  } catch (err) {
    handleError(res, err);
  }
});
...
app.post('/lf/juri', async (req, res) => {
  ...
  if (textId) {
    try {
      return res.json({ ok: true, mode: 'consult', path_used: '/consult/juri', textId, result: await callLegifrance('/consult/juri', { textId, ...(req.body?.searchedString ? { searchedString: req.body.searchedString } : {}) }) });
    } catch (err) {
      return handleError(res, err);
    }
  }
  ...
  try {
    ...
    res.json({ ok: true, mode: 'search', path_used: '/search', totalResultNumber: upstream.totalResultNumber, results: upstream.results || [], routing: { query, fond_used: fond, endpoint_called: '/lf/juri' } });
  } catch (err) {
    handleError(res, err);
  }
});
...
app.get('/lf/chrono/:textCid', async (req, res) => {
  ...
  try {
    res.json({ ok: true, path_used: '/chrono/textCid/' + textCid, result: await callLegifrance('/chrono/textCid/' + encodeURIComponent(textCid), {}, 'GET') });
  } catch (err) {
    handleError(res, err);
  }
});
