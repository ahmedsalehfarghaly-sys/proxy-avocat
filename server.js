...
const ALLOWED_FONDS = new Set(["ALL","JORF","CNIL","CETAT","JURI","JUFI","CONSTIT","KALI","CODE_DATE","CODE_ETAT","LODA_DATE","LODA_ETAT","CIRC","ACCO"]);
const ALLOWED_TYPE_CHAMP = new Set(["ALL","TITLE","ARTICLE","TEXTE","MOTS_CLES","IDCC"]);
const ALLOWED_TYPE_RECHERCHE = new Set(["TOUS_LES_MOTS_DANS_UN_CHAMP","UN_DES_MOTS","EXPRESSION_EXACTE"]);
const UNSAFE_FONDS_FOR_SAFE_SEARCH = new Set(["CODE_DATE","CODE_ETAT","LODA_DATE","LODA_ETAT"]);

function normalizeLfSearchBody(body = {}) {
  const terms = String(body.terms || body.query || "").trim();
  let fond = String(body.fond || "").trim() || "ALL";
  if (!ALLOWED_FONDS.has(fond)) fond = "ALL";
  const typeChamp = ALLOWED_TYPE_CHAMP.has(String(body.typeChamp || "").trim()) ? String(body.typeChamp).trim() : "ALL";
  const typeRecherche = ALLOWED_TYPE_RECHERCHE.has(String(body.typeRecherche || "").trim()) ? String(body.typeRecherche).trim() : "TOUS_LES_MOTS_DANS_UN_CHAMP";
    ...
    body: {
      ...
      recherche: {
        ...
        champs: [{
          typeChamp: "ALL",
          operateur: "ET",
          criteres: [{ valeur: terms, operateur: "ET", typeRecherche: "TOUS_LES_MOTS_DANS_UN_CHAMP" }]
        }]
      }
    }
  });
...
function looksLikeLongLegalQuery(query) {
  ...
  return q.split(" ").length >= 4 || /rupture brutale|relations commerciales|responsabilite civile|faute grave|forfait jours|harcelement/i.test(q);
}
...
function isMeaningfulAccoResult(r) {
  ...
  return Boolean(r.id || r.idcc || r.label || r.title) && /accord|convention|avenant|idcc|collective|temps de travail|forfait|syntec|metallurgie/i.test(label);
}
...
function simplifyAccoQuery(query) {
  return normalizeText(query).replace(/\b(convention|collective|nationale|accord|idcc)\b/g, " ").replace(/\s+/g, " ").trim() || query;
}
...
function isBroadScanQuery(q) {
  ...
  return /contrat|travail|licenciement|commerce|forfait jours|responsabilite|liberte/i.test(s) && !/\b(202[0-9]|cc|soc|comm|pourvoi|exact|ecli|\d{2}-\d{2}\.\d{3})\b/.test(s);
}

console.log("LOADED PROXY V3 LF+JD");

app.get("/health", (req, res) => {
  res.json({ ok: true, service: "proxy-avocat-v3" });
});
...
async function safeHealthcheck(baseUrl, path) {
  try {
    ...
    if (out.data?.parseError) {
      return { ok: false, status: out.status, data: { ok: false, parseError: true, raw: out.rawText || null, path } };
    }
    return out;
  } catch (e) {
    return { ok: false, status: 500, data: { ok: false, error: e.message, path } };
  }
}
...
async function handleLfSearch(req, res, options = { safeFondFallback: false }) {
  try {
    ...
    let out = await upstreamJson({ baseUrl: LF_BASE, path: "/search", method: "POST", body: bodyToSend });

    if (!out.ok && bodyToSend.fond !== "ALL") {
      bodyToSend = { ...bodyToSend, fond: "ALL" };
      usedFondFallback = true;
      out = await upstreamJson({ baseUrl: LF_BASE, path: "/search", method: "POST", body: bodyToSend });
    }

    res.status(out.status).json(withMeta({ ...out.data, usedFondFallback }, {
...
async function handleSuggestLegifrance(req, res) {
  try {
    ...
    if (!results.length && looksLikeLongLegalQuery(query)) {
      ...
      results = buildPseudoSuggestionsFromSearch(searchOut.data);
      return res.status(searchOut.status).json(withMeta({
        executionTime: searchOut.data?.executionTime ?? out.data?.executionTime ?? null,
        results,
        source_quality: "fallback_search",
        usedFallback
      }, { query }));
    }
...
async function searchAcco(query) {
  const searchOut = await upstreamJson({
    baseUrl: LF_BASE,
    path: "/search",
    method: "POST",
  ...
  const results = normalizeAccoSearchResults(Array.isArray(searchOut.data?.results) ? searchOut.data.results : []);
...
async function handleSuggestAcco(req, res) {
  try {
    ...
    if (!results.length) {
      usedFallback = true;
      let { searchOut, results: fallbackResults } = await searchAcco(query);
      ...
      if (!fallbackResults.length) {
        ...
        if (simplified && simplified !== query) {
          const second = await searchAcco(simplified);
    ...
    const out = await upstreamJson({
      ...
      body: {
        ...
        recherche: {
          ...
          operateur: "ET",
          typePagination: "DEFAUT",
          champs: [{ typeChamp: "ALL", operateur: "ET", criteres: [{ valeur: codeTerms, operateur: "ET", typeRecherche: "TOUS_LES_MOTS_DANS_UN_CHAMP" }] }]
        }
      }
    });
...
async function handleJudilibreDecision(req, res) {
  try {
    const out = await upstreamJson({ baseUrl: JD_BASE, path: "/decision", method: "GET", query: req.query });
...
app.listen(PORT, () => {
  console.log(`Proxy V3 LF + JD en écoute sur ${PORT}`);
});
