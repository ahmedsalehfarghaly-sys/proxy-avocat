...
app.get("/", (_req, res) => {
  res.json({ ok: true, service: "proxy-adaptateur-complet" });
});
...
app.post("/lf/search-structured", async (req, res) => {
  try {
    const result = await upstreamRequest({
      api: "legifrance",
      method: "POST",
...
app.post("/lf/code", async (req, res) => {
  try {
    const {
      textId,
      date = "2024-01-01",
...
app.listen(PORT, () => {
  console.log(`Proxy adaptateur complet lancé sur http://localhost:${PORT}`);
});
